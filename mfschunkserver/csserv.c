...
void csserv_write_init(csserventry *eptr,const uint8_t *data,uint32_t length) {
	...
	if (length>(8+4)) {	// connect to another cs
		...
		syslog(LOG_WARNING,"[fwdip = %"PRIu32"] [fwdport = %"PRIu16"]",eptr->fwdip,eptr->fwdport);
		eptr->connretrycnt = 0;
...
void csserv_write_data(csserventry *eptr,const uint8_t *data,uint32_t length) {
	...
	size = get32bit(&data);
	syslog(LOG_WARNING,"chunkid=%"PRIu64" writeid=%"PRIu32" blocknum=%"PRIu16" offset=%"PRIu16" size=%"PRIu32"",chunkid,writeid,blocknum,offset,size);
