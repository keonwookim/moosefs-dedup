...
void masterconn_replicate(masterconn *eptr,const uint8_t *data,uint32_t length) {
	...
	uint8_t *ptr;
	void *packet;
	//syslog(LOG_NOTICE,"replicate!!!!!!!!!1\n");
	syslog(LOG_NOTICE,"MATOCS_REPLICATE - call masterconn_replicate");
...
//	uint16_t port;
	uint8_t *ptr;
	//syslog(LOG_NOTICE,"replicate!!!!!!!!!1\n");
	syslog(LOG_WARNING,"MATOCS_REPLICATE - call masterconn_replicate2");
...
void masterconn_gotpacket(masterconn *eptr,uint32_t type,const uint8_t *data,uint32_t length) {
	switch (type) {
		...
		case MATOCS_REPLICATE:
			syslog(LOG_NOTICE,"MATOCA_REPLICATE\n");
