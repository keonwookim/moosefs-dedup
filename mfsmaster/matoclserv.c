...
//	endptr = ptr + length;
	...
	while ((ofptr=*ofpptr) && inode>0) {
		if (ofptr->inode<inode) {
			//;syslog(LOG_WARNING,"fuse_reserved_inodes1111!!!!!!!!!!!!!");
	...
	while ((ofptr=*ofpptr)) {
		//syslog(LOG_WARNING,"fuse_reserved_inodes2222222!!!!!!!!!!!");
		fs_release(ofptr->inode,eptr->sesdata->sessionid);
		*ofpptr = ofptr->next;
		free(ofptr);
	}
...
void matocl_session_timedout(session *sesdata) {
	...
	//syslog(LOG_WARNING,"matocl_session_timeout!!!!!!!!!!!!!!!!");
	while (fl) {
		afl = fl;
		fl=fl->next;
...
void matocl_session_check(void) {
	...
	uint32_t now;
	//syslog(LOG_WARNING,"session_check!!!!!!!!!!!!!!!");
	now = main_time();
...
*/
			...
			case CLTOMA_FUSE_WRITE_CHUNK:
				if(length == 4+4+4+4+2+4)
					matoclserv_fuse_sethash(eptr,data,length);
				else
...
//kkw
void matoclserv_fuse_sethash(matoclserventry *eptr, const uint8_t *data, uint32_t length) {
	uint8_t *ptr;
	uint8_t status;
	uint32_t inode;
	uint32_t indx;
	...
	if(length<4+4+4+2) {
		syslog(LOG_NOTICE,"CLTOMA_FUSE_SETHASH - wrong size (%"PRIu32"/12)",length);
		eptr->mode = KILL;
		return;
	}
	...
	ptr = matoclserv_createpacket(eptr,MATOCL_FUSE_WRITE_CHUNK,8);
	put32bit(&ptr,msgid);
	put32bit(&ptr,inode);
	//syslog(LOG_WARNING,"matoclserv_fuse_sethash [hash=====%"PRIu32"]",hash);
	fs_sethash(inode,indx,hash,pos,size);
	return;
}
