...
int chunk_multi_modify(uint32_t ts,uint64_t *nchunkid,uint64_t ochunkid,uint32_t inode,uint16_t indx,uint8_t goal,uint8_t opflag) {
	...
	if (ochunkid==0) {	// new chunk
		syslog(LOG_WARNING,"create chunk@@@@@@@@@@2 demand = [%"PRIu32"]",goal);
//		servcount = matocsserv_getservers_ordered(ptrs,MINMAXRND,NULL,NULL);
#ifndef METARESTORE
		servcount = matocsserv_getservers_wrandom(ptrs,goal);
		if (servcount==0) {
...
#endif
		*nchunkid = c->chunkid;
		//syslog(LOG_WARNING,"create chunk 222222222222222222");
	} else {
		//syslog(LOG_WARNING,"no create chunk!!!!!!!!!!!!!!!!!!");
		c = NULL;
	...
	for (s=c->slisthead ;s ; s=s->next) {
		if (s->valid!=INVALID && s->valid!=DEL) {
			if (cnt<100 && matocsserv_getlocation(s->ptr,&(lstab[cnt].ip),&(lstab[cnt].port))==0) {
				...
				//printf("chunkserver cnt= %"PRId8" dist=%"PRId32" !!!!!!!!!!11\n",cnt,lstab[cnt].dist);
...
//			sptr[cnt++]=s->ptr;
	...
	wptr = loc;
	for (i=0 ; i<cnt ; i++) {
		//printf("chunkserver ip=%d port=%d\n",lstab[i].ip, lstab[i].port);
...
void searchchunkhash(void* scr, void* dst, uint32_t start, uint32_t end)
{
	syslog(LOG_NOTICE,"replicate chunk start = %u end = %u\n");
	int i;
	chunk* c;
	//chunkatt ca;
	for(i = 0; i<HASHSIZE;i++)
	{
		for(c = chunkhash[i]; c; c=c->next)
		{
			if(c->crc >= start && c->crc <=end)
			{
				syslog(LOG_NOTICE,"replicate chunk\n");
				//ca.chunkid = c->chunkid;
				//ca.version = c->version;
				//call_repanddel(ca);
				matocsserv_send_replicatechunk(dst,c->chunkid,c->version,scr);
			}
		}
	}
	
	//deletechunkentry(scr,dst,start,end);
	
}
void deletechunkentry(void* scr, void* dst, uint32_t start, uint32_t end)
{
	syslog(LOG_NOTICE,"replicate chunk start = %u end = %u\n");
	int i;
	chunk* c;
	...
	for(i = 0; i<HASHSIZE;i++)
	{
		for(c = chunkhash[i]; c; c=c->next)
		{
			if(c->crc >= start && c->crc <=end)
			{
				...
				//ca.version = c->version;
				//call_repanddel(ca);
				matocsserv_send_deletechunk(scr,c->chunkid,c->version);
			}
		}
	}
}
	...
	for(i = 0; i<HASHSIZE;i++)
	{
		for(c = chunkhash[i]; c; c=c->next)
		{
			if(c->crc >= start && c->crc >=end)
			{
				...
				//call_repanddel(ca);
				matocsserv_send_replicatechunk(dst,c->chunkid,c->version,scr);
				matocsserv_send_deletechunk(dst,c->chunkid,c->version);
			}
		}
	}
}
