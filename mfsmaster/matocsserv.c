...
typedef struct conhashserventry{
	uint32_t start;
	uint32_t end;
	uint32_t size;
//	uint32_t ip;
	matocsserventry *node;
	struct conhashserventry *next;
}conhashserventry;

static conhashserventry *conhashservhead = NULL;
static conhashserventry *conhashservtail = NULL;
static uint64_t maxtotalspace;
static matocsserventry *matocsservhead=NULL;
static matocsserventry *source = NULL;
static matocsserventry *dest = NULL;
static int lsock;
static int32_t lsockpdescpos;
static uint32_t hashstart =0;
static uint32_t hashend = 0;
// from config
...
//kkw2
conhashserventry* initnode();
conhashserventry* createnode(conhashserventry* node, uint32_t start, uint32_t end, uint32_t size, matocsserventry *matocsnode);
void addnode(conhashserventry **head, conhashserventry *node, conhashserventry* pnode, uint8_t isequal);
void searchlocation(conhashserventry **head, matocsserventry *node);
void searchchunk(matocsserventry* scr,matocsserventry* dst, uint32_t start, uint32_t end);
...
void matocsserv_replication_end(uint64_t chunkid,uint32_t version,void *dst) {
	...
	//kkw2
	matocsserv_send_deletechunk(source,chunkid,version);
}
...
uint16_t matocsserv_getservers_wrandom(void* ptrs[65536],uint16_t demand){
	...
	temp = conhashservhead->next;
	do
	{
		if(temp->start < hash && temp->end > hash)
		{
			ptrs[i] = temp->node;
		...
		temp = temp->next;
	}while(temp!= conhashservtail||temp != NULL);
	for(i=1; i<demand; i++)
	{
		//syslog(LOG_NOTICE,"in for statement\n");
		temp = temp->next;
		ptrs[i] = temp->node;
	}
	return demand;
...
uint16_t matocsserv_getservers_wrandom1(void* ptrs[65536],uint16_t demand) {
static struct rservsort {
	...
	uint32_t chk=0;
	hash = fs_gethash();
	//syslog(LOG_WARNING,"getservers_wrandom [hash=======%"PRIu32"]",hash);
	if (maxtotalspace==0) {
		return 0;
	}
	allcnt=0;
	...
	for (eptr = matocsservhead ; eptr && allcnt<65536 ; eptr=eptr->next) {
		if (eptr->mode!=KILL && eptr->totalspace>0 && eptr->usedspace<=eptr->totalspace && (eptr->totalspace - eptr->usedspace)>MFSCHUNKSIZE) {
			...
			servtab[allcnt].s_cnt = allcnt;
			servtab[allcnt].ptr = eptr;
			servtab[allcnt].avail = 0;
			//syslog(LOG_WARNING,"chunkserver ip = %x!!!!!!!!!!!!!",eptr->servip);
			//printf("maxtotalspace = %"PRIu64"!!!!!!!!!!!\n",maxtotalspace);
			//printf("server count = %"PRIu32"!!!!!!!!!!!!1\n",servtab[allcnt].s_cnt);
			allcnt++;
	...
	//printf("shift hash = %"PRIu32"!!!!!!!!!!!!!!!!1\n",(hash>>1));
	//printf("demand =  %"PRIu16"!!!!!!!!!!!!!!!!\n",demand);
	//printf("select server = %"PRIu32"!!!!!!!!!!!!!!!\n",((hash>>1)%allcnt));
	//syslog(LOG_WARNING,"all server count [count =====%"PRIu32"]",allcnt);
	if(demand>allcnt){
		demand=allcnt;
	}
	/*replication1
	for(i=0;i<allcnt;i++){
		if(servtab[i].s_cnt == (hash%allcnt)){
			ptrs[0] = servtab[i].ptr;
			//syslog(LOG_WARNING,"server number ====[%"PRIu32"]",servtab[i].s_cnt);
			break;
		}
	}
	*/
	syslog(LOG_WARNING,"demand = [%"PRIu32"]  allcnt = [%"PRIu32"]",demand,allcnt);
	if(demand == 1) {
		modval[0] = (hash>>i)%allcnt;
	} else{
		for(i=0; i<allcnt ;i++)//demand로 바꿔
		{
			modval[i] = (hash>>i)%allcnt;
			syslog(LOG_WARNING,"modval = [%"PRIu32"]",modval[i]);
			cnt++;
		}
		//qsort(modval,demand,sizeof(uint32_t),matocsserv_compare);
		for(i=0;i<demand;i++){
			syslog(LOG_WARNING,"sorted modval = [%"PRIu32"]",modval[i]);
		}
		while(check(modval,allcnt,hash,&cnt)==1);
/*
		while(chk==0){
			for(i=0;i<allcnt-1;i++){
				for(j=i+1;j<allcnt;j++){

					while(modval[i] == modval[j]){
						cnt++;
						modval[j] = (hash>>cnt)%allcnt;
						chk = 1;
					}
				}
			}
...
*/
	...
	for(i=0;i<demand;i++){
		syslog(LOG_WARNING,"real modval = [%"PRIu32"]",modval[i]);
	}
	/*for(i=0; i<demand ;i++)
	{
		uint32_t sameserver=0;
		for(j=0; j<allcnt;j++){
			sameserver = 0;
			if(servtab[j].s_cnt == ((hash>>cnt)%allcnt)){
				for(k=0; k < i ; k++){
					if(demandserv[k] == ((hash>>cnt)%allcnt)){
						sameserver = 1;
						cnt++;
						j--;
						break;
				...
				if(sameserver == 0){
					demandserv[i] = ((hash>>cnt)%allcnt);
					ptrs[i] = servtab[j].ptr;
					cnt++;
					break;
				}
			}
	...
	syslog(LOG_WARNING,"count = %"PRIu32"",cnt);
	for(i=0;i<demand ;i++)
	{
		syslog(LOG_WARNING,"selected server num ========[num=%"PRIu32"]",servtab[i].s_cnt);
		syslog(LOG_WARNING,"demand server ========== %"PRIu32"",demandserv[i]);
	}
	...
	if(cnt < 32) {
		...
		while (availcnt<demand) {
			...
			for (i=0 ; i<allcnt ; i++) {
				...
				//printf("matocsserv_getservers_wrandom:w=%lf!!!!!!!!",servtab[allcnt].w);
				//printf("matocsserv_getservers_wrandom:carry=%lf!!!!!!1",servtab[allcnt].carry);
				if (carry>=1.0) {
					availcnt++;
				}
			}
		...
		for (i=0 ; i<demand ; i++) {
			...
			//printf("real carry = %lf!!!!!!!!!!!!!!!11111",servtab[i].carry);
		}
	}
	return demand;
}
int check(int modval[],uint32_t allcnt,uint32_t hash,uint32_t *cnt){
	int i,j;
	int chk=0;
	for(i=0;i<(allcnt-1);i++){
		for(j=i+1;j<allcnt;j++){
			syslog(LOG_WARNING,"i = [%d], j = [%d], cnt = [%"PRIu32"]",i,j,*cnt);
			while(modval[i]==modval[j])
			{
				if((*cnt)==32)
					break;
...
int matocsserv_send_deletechunk(void *e,uint64_t chunkid,uint32_t version) {
	matocsserventry *eptr = (matocsserventry *)e;
	uint8_t *data;
	syslog(LOG_NOTICE,"delete chunk!!!\n");
	if (eptr->mode!=KILL) {
...
//kkw2 - make replicatechunk2  not *e, *src
int matocsserv_send_replicatechunk(void *e,uint64_t chunkid,uint32_t version,void *src) {
	...
	if (matocsserv_replication_find(chunkid,version,eptr)) {
		syslog(LOG_NOTICE,"chunk not found!\n");
		return -1;
		
	}
	syslog(LOG_NOTICE,"chunk found!\n");
...
void matocsserv_register(matocsserventry *eptr,const uint8_t *data,uint32_t length) {
	...
	if ((length&1)==0) {
		...
		if (rversion==1) {
			...
			//kkw2
			syslog(LOG_NOTICE,"call searchlocation\n");
			searchlocation(&conhashservhead,eptr);
			//syslog(LOG_NOTICE,"start %u!!!!!!!!!!!!!!!!11111\n",conhashservhead->next->end);
...
//		eptr->duplication = NULL;
	...
	//kkw2
	//searchlocation(&conhashservhead, eptr);
	//syslog(LOG_NOTICE,"start %u\n!!!!!!!!!!!!!!!!11111",conhashservhead->next->start);
}
...
void matocsserv_gotpacket(matocsserventry *eptr,uint32_t type,const uint8_t *data,uint32_t length) {
	switch (type) {
		...
		case CSTOMA_DUPTRUNC:
			...
			break;
		default:
			syslog(LOG_NOTICE,"matocs: got unknown message (type:%"PRIu32")",type);
			eptr->mode=KILL;
	}
}
...
//				eptr->replication=NULL;
			//kkw2
			//if(conhashservhead == NULL)
			//	conhashservhead = (conhashserventry*)malloc(sizeof(conhashserventry));
...
//			if (FD_ISSET(eptr->sock,wset) && eptr->mode!=KILL) {
	...
	while ((eptr=*kptr)) {
		if (eptr->mode == KILL) {
			...
			deletenode(&conhashservhead,eptr);
			tcpclose(eptr->sock);
			if (eptr->inputpacket.packet) {
				free(eptr->inputpacket.packet);
			}
...
int matocsserv_init(void) {
	...
	matocsservhead = NULL;
	//kkw2
	conhashservhead = NULL;
			if(conhashservhead == NULL)
			{
				syslog(LOG_NOTICE,"init!!!\n");
				conhashservhead = (conhashserventry*)malloc(sizeof(conhashserventry));
				conhashservhead->next = NULL;
			}
...
conhashserventry* initnode()
{
	syslog(LOG_NOTICE,"initnode\n");
	conhashserventry* node;
	node = (conhashserventry*)malloc(sizeof(conhashserventry));
	node->start = 0;
	node->end = 0;
	node->size = 0;
	node->node = NULL;
...
conhashserventry* createnode(conhashserventry* node, uint32_t start, uint32_t end, uint32_t size, matocsserventry* matocsnode)
{
	syslog(LOG_NOTICE,"createnode\n");
	node->start = start;
	node->end = end;
	node->size = size;
	node->node = matocsnode;
	return node;
}

void addnode(conhashserventry **head, conhashserventry *node, conhashserventry* pnode, uint8_t isequal)
{
	syslog(LOG_NOTICE,"addnode\n");
	if((*head)->next == NULL)
	{
		(*head)->next = node;
		conhashservtail = node;
		node->next = NULL;
	}
	...
	else if(isequal == 0)
	{
		node->next = pnode->next;
		pnode->next = node;
		if(pnode->next == conhashservhead->next)
		{
			node->next = (*head)->next;
			conhashservtail = node;
		}
	}
	else if(pnode->next == conhashservhead->next)
	{
		node->next = (*head)->next;
		pnode->next = node;
		conhashservtail = node;
...
void deletenode(conhashserventry **head, matocsserventry *node)
{
	conhashserventry *temp = (*head)->next;
	conhashserventry *pnode,*cnode;
	syslog(LOG_NOTICE,"delete node");
	while(temp != conhashservtail)
	{
		if((*head)->node == node)
		{
			syslog(LOG_NOTICE,"delete node1");
			temp->start = 0;
			temp->end = 0;
			temp->size = 0;
			break;
		...
		{
			syslog(LOG_NOTICE,"delete node2");
			cnode = temp->next;
			pnode = temp;	
			pnode->end = cnode->end;
			pnode->size = pnode->size * 2;
			cnode->start = 0;
			cnode->end = 0;
			cnode->size = 0;
			break;
...
void searchlocation(conhashserventry** head, matocsserventry* node)
{
	syslog(LOG_NOTICE,"searchlocation\n");
	conhashserventry *temp;
	int size = 0;
	int start = 0;
	int end = 0;
	...
	if((*head)->next == NULL)
	{
		syslog(LOG_NOTICE,"empty\n");
		temp = initnode();
		createnode(temp,0,4294967295,4294967295,node);//////////
		syslog(LOG_WARNING,"start %u end %u ip %s\n",temp->start,temp->end,temp->node->servstrip);
		addnode(&conhashservhead,temp,NULL,0);
		
	}
	else if((*head)->next->next == NULL)
	{
		syslog(LOG_NOTICE,"one node\n");
		temp = initnode();
		size = (*head)->next->size/2;
		start = size+1;
		end = (*head)->next->end;
		(*head)->next->size = size;
		(*head)->next->end = size;
		createnode(temp,start,end,size,node);///////////////
		addnode(&conhashservhead,temp,(*head)->next,0);
		syslog(LOG_WARNING,"start %u end %u ip %s\n",temp->start,temp->end,temp->node->servstrip);
		source = (*head)->next->node;
		dest = temp->node;
		hashstart = start;
		hashend = end;
		searchchunk((*head)->next->node,temp->node,start,end);
	}
	else
	{
		syslog(LOG_NOTICE,"else\n");
		conhashserventry *temp1 = conhashservhead->next;
		while(temp1 != conhashservtail)
		{
			if(temp1->size != temp1->next->size)
			{
				isequal=0;
				break;
			}
		...
		{
			temp = initnode();
			temp1 = conhashservhead->next;
			size = temp1->size/2;
			start = size+1;
			end = start + size;
			temp1->size = size;
			temp1->end = temp1->start + size;
			createnode(temp,start,end,size,node);//////////////
			addnode(&conhashservhead, temp, (*head)->next,1);
			source = (*head)->next->node;
			dest = temp->node;
			hashstart = start;
			hashend = end;
		...
		{
			temp = initnode();
			size = temp1->next->size/2;
			start = temp1->next->start + size +1;
			end = start + size;
			temp1->next->size = size;
			temp1->next->end = temp1->next->start + size;
			createnode(temp,start,end,size,node);///////////////
			addnode(&conhashservhead, temp, temp1->next,0);
			source = temp1->next->node;
			dest = temp->node;
			hashstart = start;
			hashend = end;
			searchchunk(temp1->next->node,temp->node,start,end);
		}
	}
}
...
void searchchunk(matocsserventry* scr,matocsserventry* dst, uint32_t start, uint32_t end)
{
	syslog(LOG_NOTICE,"scr : %s dst : %s\n",scr->servstrip, dst->servstrip);
	void *scr1 = scr;
	void *dst1 = dst;
	syslog(LOG_NOTICE,"call searchchunk\n");
	syslog(LOG_NOTICE,"start = %u end = %u\n");
	searchchunkhash(scr1,dst1,start,end);
}
void deletechunk(matocsserventry* scr,matocsserventry* dst, uint32_t start,uint32_t end)
{
	void *scr1 = scr;
	void *dst1 = dst;
	deletechunkentry(scr1,dst1,start,end);
}
//void call_repanddel(chunkatt ca)
//{
