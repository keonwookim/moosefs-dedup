...
void* fs_nop_thread(void *arg) {
	...
	(void)arg;
	//syslog(LOG_WARNING,"lastwrite == [%s]",ctime(&lastwrite));
	for (;;) {
		//syslog(LOG_WARNING,"lastwrite == [%s]",ctime(&lastwrite));
		//syslog(LOG_WARNING,"%d inodeswritecnt ",inodeswritecnt);
		//syslog(LOG_WARNING,"disconnect ==[%d]",disconnect);
		now = time(NULL);
		pthread_mutex_lock(&fdlock);
		if (fterm) {
			if (fd>=0) {
		...
		if (disconnect==0 && fd>=0) {
			...
			//kkw
			for(afptr=afhead ; afptr ; afptr=afptr->next){
				//syslog(LOG_WARNING,"reserved inode0000 : %"PRIu32"",afptr->inode);
			}
			...
			if (inodeswritecnt==0) {	// HELD INODES
				...
				for (afptr=afhead ; afptr ; afptr=afptr->next) {
					//syslog(LOG_WARNING,"disconnect ==[%d]",disconnect);
					//syslog(LOG_WARNING,"reserved inode: %"PRIu32"",afptr->inode);
					inodesleng+=4;
				}
				...
				for (afptr=afhead ; afptr ; afptr=afptr->next) {
					...
					syslog(LOG_WARNING,"reserved inode2: %"PRIu32"",afptr->inode);
				}
...
uint8_t fs_sethash(uint32_t inode, uint32_t chindx, uint16_t pos, uint32_t from, uint32_t to, uint32_t hash){
	uint8_t *wptr;
	const uint8_t *rptr;
	uint32_t i;
	uint8_t ret;
	...
	wptr = fs_createpacket(rec,CLTOMA_FUSE_WRITE_CHUNK,4+2+4+4+4);
