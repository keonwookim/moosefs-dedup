...
//	inodedata *id;
	...
	for (;;) {
		for (cnt=0 ; cnt<chainelements ; cnt++) {
			...
			syslog(LOG_WARNING,"[csdb ip = %"PRIu8"] [port = %"PRIu8"] [chainelements = %"PRIu16"]==> in write_worker\n",chainip[cnt],chainport[cnt]);
		}
		chainelements=0;

		// get next job
		queue_get(jqueue,&z1,&z2,&data,&z3);
		//syslog(LOG_WARNING,"writedata     %s!!!!!!!!!!!!!!!!",data);
		if (data==NULL) {
			return NULL;
		}
		id = (inodedata*)data;
		...
		//printf("file %"PRIu32", index: %"PRIu32"!!!!!!!!!!!!!!!1",id->inode,chindx);
		//syslog(LOG_WARNING,"file %"PRIu32", index %"PRIu32"!!!!!!!!!!!1",id->inode,chindx);
...
// debug:				syslog(LOG_NOTICE,"writeworker: data packet prepared (writeid:%"PRIu32",pos:%"PRIu16")",cb->writeid,cb->pos);
					...
					//syslog(LOG_WARNING,"chunk id : %"PRIu64" writeid :  %"PRIu32" pos : %"PRIu16" from %"PRIu16" to-from : %"PRIu32"!!!!!!!!!",chunkid,cb->writeid,cb->pos,cb->from,cb->to-cb->from);
#ifdef WORKER_DEBUG
...
//	id = write_get_inodedata(inode);
	...
	//syslog(LOG_WARNING,"=========================================================");
	//syslog(LOG_WARNING,"size=%"PRIu32" offset=%"PRIu64" chunkindex=%"PRIu32" pos=%"PRIu16" from=%"PRIu32"!!!!!!!!",size,offset,chindx,pos,from);
	//syslog(LOG_WARNING,"%s",data);
	while (size>0) {
